...
class EKYCController {
  ...
  static submitEKYC = async (req, res) => {
    try {
  ...
  static getEKYCByApplicationId = async (req, res) => {
    try {
      ...
      console.error('Get E-KYC Error:', error);
      res.status(500).json({
        success: false,
  ...
  // USER: Get User's E-KYC Forms
  static getMyEKYCForms = async (req, res) => {
    try {
      const userId = req.user._id;

      const ekycForms = await EKYC.find({ submittedBy: userId })
        .sort({ createdAt: -1 });
      ...
      console.error('Get My E-KYC Forms Error:', error);
      res.status(500).json({
        success: false,
  ...
  static getAllEKYCForms = async (req, res) => {
    try {
      const {
        status,
        state,
      ...
      console.error('Get All E-KYC Forms Error:', error);
      res.status(500).json({
        success: false,
        message: 'Failed to fetch E-KYC forms',
        error: error.message
  ...
  // ADMIN: Get E-KYC by ID
  static getEKYCById = async (req, res) => {
    try {
      const { id } = req.params;
      ...
      console.error('Get E-KYC by ID Error:', error);
      res.status(500).json({
        success: false,
        message: 'Failed to fetch E-KYC record',
        error: error.message
  ...
  // ADMIN: Update E-KYC Status (Approve/Reject)
  static updateEKYCStatus = async (req, res) => {
    try {
      const { id } = req.params;
  ...
  // ADMIN: Get E-KYC Statistics
  static getEKYCStatistics = async (req, res) => {
    try {
      const total = await EKYC.countDocuments();
      const pending = await EKYC.countDocuments({ status: 'pending' });
      const submitted = await EKYC.countDocuments({ status: 'submitted' });
      const verified = await EKYC.countDocuments({ status: 'verified' });
      const rejected = await EKYC.countDocuments({ status: 'rejected' });

      // Get state-wise distribution
      const stateDistribution = await EKYC.aggregate([
        {
          $group: {
            _id: '$state',
            count: { $sum: 1 }
      ...
      const recentSubmissions = await EKYC.find()
        .sort({ createdAt: -1 })
        .limit(5)
        .select('applicationId name status createdAt');
      ...
      console.error('Get E-KYC Statistics Error:', error);
      res.status(500).json({
        success: false,
        message: 'Failed to fetch E-KYC statistics',
        error: error.message
      });
    }
  };

  // ADMIN: Delete E-KYC Record
  static deleteEKYC = async (req, res) => {
    try {
      const { id } = req.params;
      ...
      console.error('Delete E-KYC Error:', error);
      res.status(500).json({
        success: false,
  ...
  static bulkUpdateStatus = async (req, res) => {
    try {
      ...
      const result = await EKYC.updateMany(
        { _id: { $in: ids } },
        { $set: updateData }
      );
  ...
  // ADMIN: Export E-KYC Data (CSV)
  static exportEKYCData = async (req, res) => {
    try {
      ...
      // Convert to CSV format (you can use a library like 'json2csv' for this)
      res.status(200).json({
        success: true,
      ...
      console.error('Export E-KYC Data Error:', error);
      res.status(500).json({
        success: false,
